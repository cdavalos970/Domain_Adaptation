...
def neural_network(train, dev, test, iter = ITER):
    ...
    for i in range(1,iter + 1):
        ...
        alpha = random.uniform(0.0001, 0.9)
        batch_size = random.choice([200, 300, 400, 500])
        learning_rate = random.choice([0.001, 0.001, 0.01, 0.1])
        mlp = MLPRegressor(hidden_layer_sizes=hidden_layer_sizes, activation='relu', alpha = alpha, batch_size=batch_size, learning_rate_init=learning_rate, max_iter=2000)
        ...
        for j in range(0, len(dev)):
            ...
            mse = mean_squared_error(y_dev, y_pred_cv)
            out = ('Neural Network', hidden_layer_sizes, alpha,  batch_size, learning_rate, mse, i, j)
            output = pd.DataFrame([out], columns=('model', 'hidden_layers', 'alpha', 'batch_size', 'learning_rate', 'mse', 'iter', 'fold'))
            summary = summary.append(output)
    summary_final = pd.DataFrame(summary.groupby(['model', 'hidden_layers', 'alpha', 'batch_size', 'learning_rate', 'iter']).mean().sort_values(by=['mse']).reset_index())
    best_hidden = summary_final['hidden_layers'].iloc[0]
    best_alpha = summary_final['alpha'].iloc[0]
    best_batch_size = summary_final['batch_size'].iloc[0]
    best_learning_rate = summary_final['learning_rate'].iloc[0]
    mlp = MLPRegressor(hidden_layer_sizes=best_hidden, activation='relu', alpha = best_alpha, batch_size = best_batch_size, learning_rate_init = best_learning_rate, max_iter=2000)
    mlp.fit(X_train, y_train.values.ravel())
...
def main():
    ...
    summary_final.to_csv('summary_FEDA_test_ratio_lasso.csv', index=False, sep = ',')
    all_data_final.to_csv('FEDA_test_ratio_lasso.csv', index=False, sep = ',')
