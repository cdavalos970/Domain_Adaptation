...
def cross_validation_set(replica_number, input, percentage_dev = PER_DEV, percentage_test = PER_TEST, samples = SAMPLES, folds = FOLDS):
    ...
    if replica_number == 'female':
        ...
        train = pd.concat([train_female.sample(n = samples),male,mixed])
        ...
        train = pd.concat([female,train_male.sample(n = samples),mixed])
        ...
        train = pd.concat([female,male,train_mixed.sample(n = samples)])
...
def linear_regression(train, dev, test, iter = ITER):
    ...
    for i in range(1,iter + 1):
        ...
        clf = linear_model.Lasso(random_state=0, alpha = alpha).fit(X_train, y_train.values.ravel())
    ...
    clf = linear_model.Lasso(random_state=0, alpha = best_alpha).fit(X_train, y_train.values.ravel())
...
def cross_validation(input):
    ...
    for i in ['female', 'male','mixed']:
        train, dev, test = cross_validation_set(i, input)

        mse = linear_regression(train, dev, test)
        mse['target'] = i
        summary_final = summary_final.append(mse)

        mse  = neural_network(train, dev, test)
        mse['target'] = i
        summary_final = summary_final.append(mse)

        mse  = support_vector(train, dev, test)
        mse['target'] = i
        summary_final = summary_final.append(mse)
...
def main():
    ...
    summary_final = cross_validation(input)
    summary_final['algorithm'] = 'ALL_' + str(SAMPLES)
    summary_final.to_csv('summary_ALL_' + str(SAMPLES) + '.csv', index=False, sep = ',')
