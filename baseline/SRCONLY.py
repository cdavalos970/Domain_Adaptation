...
def cross_validation_set(replica_number, input, percentage_dev = PER_DEV, percentage_test = PER_TEST, samples = SAMPLES, folds = FOLDS):
    ...
    if replica_number == 'female':
        ...
        train = pd.concat([male,mixed]).sample(n = samples)
        ...
        train = pd.concat([female,mixed]).sample(n = samples)
        ...
        train = pd.concat([female,male]).sample(n = samples)
...
def linear_regression(train, dev, test, iter = ITER):
    ...
    X_test = train.drop(columns=['type','Exam Score'])
    y_test = train[['Exam Score']]
...
def main():
    ...
    summary['algorithm'] = 'SRCONLY_' + str(SAMPLES)
    summary.to_csv('summary_SRCONLY_' + str(SAMPLES) + '.csv', index=False, sep = ',')
